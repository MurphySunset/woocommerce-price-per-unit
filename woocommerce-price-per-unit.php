        ...
        if (!empty($coefficient) && is_product(true) ) {
            ...
            if ($product->is_type('variable')) {
                ...
                if ($product->is_on_sale()) {
                    return $price . '<div class="price-per-unit"><del>' . number_format($variation_regular_price / $coefficient, 2, ",", " "). $currency_symbol . ' / ' . $unit . '</del>
                    <ins>' . number_format($variation_min_price / $coefficient, 2, ",", " "). $currency_symbol . ' / ' . $unit . '</ins></div>' ;
                }
                else {
                    return $price . '<div class="price-per-unit"><ins>' . number_format($variation_min_price / $coefficient, 2, ",", " "). $currency_symbol . ' / ' . $unit . '</ins></div>' ;
                }
            } else {
                if ($product->is_on_sale()) {
                    return $price . '<div class="price-per-unit"><del>' . number_format($regular_price / $coefficient, 2, ",", " "). $currency_symbol . ' / ' . $unit . '</del>
                    <ins>' . number_format($sale_price / $coefficient, 2, ",", " "). $currency_symbol . ' / ' . $unit . '</ins></div>' ;
                }
                else {
                    return $price . '<div class="price-per-unit"><ins>' . number_format($regular_price / $coefficient, 2, ",", " "). $currency_symbol . ' / ' . $unit . '</ins></div>' ;
                }
            }
...
function price_per_unit_meta_box( $meta_boxes ) {
    $prefix = '';
    $unit = get_post_meta($post, 'kw_UNITE', true);
    $coefficient = get_post_meta($post, 'kw_COEFF_UNITE', true);
